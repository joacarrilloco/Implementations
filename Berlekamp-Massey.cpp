...
namespace linear_seq 
{
	...
	{// finds shortest linear recurrence given first x terms in O(x^2)
	...
	{  // a * b mod f; f = x ** m - sum{1..m} x**(m-j) * pj  
		...
		for( int i = 2 * m - 1; i >= m; -- i )
			if( r[i] )
				for( int j = m-1; j >= 0; -- j )
					r[i-j-1] = ( r[i-j-1] + 1LL * p[j] * r[i] ) % mod;

		r.resize(m);
		return r;
	}
	// O(m*m*log(k)) with Fourier O(m*log(m)*log(k))
	inline int calc( long long k ) 
	{ // res = G[ x**k ] = G[ x ** k mod f] 
		if( m == 0 ) return 0;
		vector< int > bs ( m ), r( m );
